...
const ManagePresetsPage = () => {
    ...
    const handleDeletePreset = async () => { if (!presetToDelete) return; await deleteDoc(doc(db, 'users', HOUSEHOLD_ID, 'presets', presetToDelete)); setIsModalOpen(false); setPresetToDelete(null); };

    useEffect(() => { if (user) { const q = query(collection(db, 'users', HOUSEHOLD_ID, 'presets'), orderBy('description')); const unsubscribe = onSnapshot(q, (snapshot) => { setPresets(snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }))); }); return () => unsubscribe(); } }, [user]);
    ...
    const handleEditClick = (preset) => { setEditingPresetId(preset.id); setDescription(preset.description); setAmount(preset.amount); setType(preset.type); setPerson(preset.person); setCategory(preset.category); setIsVariable(preset.isVariable); setPaymentMethod(preset.paymentMethod || paymentMethods[0]); window.scrollTo({ top: 0, behavior: 'smooth' }); };
            ...
            <div className="page-content">
                    ...
                    <form className="transaction-form" onSubmit={handleFormSubmit}>
                        ...
                        {type === 'expense' && (
                            <div className="form-control">
                                <label htmlFor="preset-paymentMethod">Payment Method</label>
                                <div className="custom-select-wrapper">
                                    <select id="preset-paymentMethod" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
                ...
                <section className="list-section">
                    ...
                    <ul className="presets-list">
                        {filteredPresets.map(preset => (<li key={preset.id}><div className="preset-info"><span className="preset-description">{preset.description}</span><span className="preset-details">{preset.category} - {preset.person} - {preset.isVariable ? 'Variable' : `€${preset.amount.toLocaleString('de-DE', {minimumFractionDigits: 2})}`}</span></div><div className="preset-actions"><button className="edit-btn" onClick={() => handleEditClick(preset)}>✏️</button><button className="delete-btn-preset" onClick={() => openDeleteModal(preset.id)}>🗑️</button></div></li>))}</ul>
                </section>
            </div>
        </>
    );
