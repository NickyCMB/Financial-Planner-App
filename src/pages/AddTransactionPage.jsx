...
import { householdMembers, incomeCategories, expenseCategories, ADMIN_EMAIL, HOUSEHOLD_ID, paymentMethods } from '../config.js';
...
const AddTransactionPage = () => {
    ...
    const handleAddTransaction = async (e) => {
        e.preventDefault();
        if (!user || !description || !amount) { alert('Bitte Beschreibung und Betrag eingeben.'); return; }
    ...
    return (
        <div className="page-content">
            <form className="transaction-form" onSubmit={handleAddTransaction}>
                ...
                {type === 'expense' && (
                    <div className="form-control"><label htmlFor="paymentMethod">Payment Method</label><div className="custom-select-wrapper"><select id="paymentMethod" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>{paymentMethods.map(method => (<option key={method} value={method}>{method}</option>))}</select></div></div>
                )}
