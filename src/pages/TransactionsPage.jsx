...
import { householdMembers, incomeCategories, expenseCategories, PERSON_COLORS } from '../config.js';
...
const TransactionsPage = () => {
    ...
    const handleDeleteTransaction = async () => { if (!user || !transactionToDelete) return; try { await deleteDoc(doc(db, 'users', user.uid, 'transactions', transactionToDelete)); } catch (error) { console.error("Error deleting document: ", error); } setIsModalOpen(false); setTransactionToDelete(null); };
    ...
    return (
        <>
            ...
            <div className="page-content">
                <div className="section-header"><h3>Balance for this view: {formatCurrency(balance)}</h3></div>
                ...
                <ul className="transaction-list">{filteredTransactions.map((transaction) => { const colors = PERSON_COLORS[transaction.person] || {}; return (<li key={transaction.id} style={{ border: `1px solid ${colors.backgroundLogged}`, backgroundColor: '#ffffff' }}><div className="transaction-details-wrapper"><div className="transaction-details"><div><span className="transaction-person" style={{ color: colors.primary }}>{transaction.person}</span><span>{transaction.description}</span></div></div><div className="transaction-meta"><span className="transaction-month">{transaction.createdAt.toDate().toLocaleString('de-DE', { month: 'long' })}</span><span className="transaction-amount" style={{ color: transaction.type === 'income' ? '#008000' : '#FF0000' }}>{transaction.type === 'income' ? '+' : ''}{formatCurrency(transaction.amount)}</span></div></div><button className="delete-btn" onClick={() => openDeleteModal(transaction.id)}>X</button></li>);})}</ul>
            </div>
        </>
    );
};
