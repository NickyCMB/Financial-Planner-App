...
const TransactionsPage = () => {
    ...
    // NEW: State for the new income/expense filter
    ...
    // UPDATED: The filter logic now includes the new typeFilter
        ...
        <>
            ...
            <div className="page-content">
                ...
                <div className="filter-controls">
                    ...
                    <div className="custom-select-wrapper month-filter"><select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)}><option value="All">All Months</option>{uniqueMonths.map(month => (<option key={month} value={month}>{new Date(month + '-02').toLocaleString('de-DE', { month: 'long', year: 'numeric' })}</option>))}</select></div>
                    <div className="custom-select-wrapper category-filter"><select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}><option value="All">All Categories</option>{allCategories.map(cat => (<option key={cat} value={cat}>{cat}</option>))} </select></div>
                    {/* --- NEW DROPDOWN FILTER --- */}
                    <div className="custom-select-wrapper type-filter">
                        ...
                        return (
                            <li key={transaction.id} style={{ backgroundColor: '#ffffff', border: `1px solid ${colors.backgroundLogged}` }}>
                                <div className="transaction-main-content">
                                    <div className="transaction-details"><div><span className="transaction-person" style={{ color: colors.primary }}>{transaction.person}</span><span>{transaction.description}</span></div></div>
                                    <div className="transaction-meta"><span className="transaction-month">{transaction.createdAt.toDate().toLocaleString('de-DE', { month: 'long', year: 'numeric' })}</span></div>
                                </div>
                                <div className="transaction-side-content">
                                    <div className="transaction-actions"><Link to={`/edit/${transaction.id}`} className="edit-btn">✏️</Link><button className="delete-btn" onClick={() => openDeleteModal(transaction.id)}>X</button></div>
                                    <span className="transaction-amount" style={{ color: transaction.type === 'income' ? '#008000' : '#FF0000' }}>{transaction.type === 'income' ? '+' : '-'}{formatCurrency(transaction.amount)}</span>
                                </div>
                            </li>
                        );
                    })}
