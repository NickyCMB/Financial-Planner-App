...
const PresetsPage = () => {
    ...
    const handleLogPreset = async (preset) => {
        ...
        try {
            await addDoc(collection(db, 'users', HOUSEHOLD_ID, 'transactions'), { description: preset.description, amount: presetAmount, type: preset.type, person: preset.person, category: preset.category, createdAt: new Date() });
                    ...
                    {checklistItems.map(item => {
                        ...
                        return (
                            <li key={item.id} className={item.isLogged ? 'logged' : ''} style={itemStyle}>
                                <div className="preset-info"><span className="preset-description">{item.description}</span></div>
